            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              <div>
                ...
                <p style={{ margin: 0, color: "#888" }}>
                  Color: {item.productDetailsColor} / Size: {item.productDetailsSize.replace('SIZE_', '')}
                </p>
...
const Review = ({ productID }) => {
  ...
  const getRatingByProduct = async () => {
    if (productID) {
      try {
        const response = await getRating(productID);
        setAverageRating(response)
      } catch (error) {
        console.error("Error fetch rating:", error);
      }
    }
  };
